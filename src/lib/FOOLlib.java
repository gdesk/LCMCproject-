...
//	}
		...
		/* Valuta se "a" sottotipo di "b" (tipo "a" <= tipo "b") */
		public static boolean isSubtype(Node a, Node b) {
			
			/* Controllo sui tipi riferiemnto */
			if(a instanceof EmptyTypeNode && b instanceof RefTypeNode) {
				return true;
			}
			if(a instanceof RefTypeNode && b instanceof RefTypeNode) {
				return classIsSubtype((RefTypeNode) a, (RefTypeNode) b);  // funzione di supporto che cerca nella HashMap<String,String> "superType"
			}
			
			/* Controllo sui tipi funzionali (arrow) */
			if (a instanceof ArrowTypeNode && b instanceof ArrowTypeNode) {
				ArrowTypeNode arrowTypeA = (ArrowTypeNode) a;
				ArrowTypeNode arrowTypeB = (ArrowTypeNode) b;
				ArrayList<Node> parListA = arrowTypeA.getParList();
				ArrayList<Node> parListB = arrowTypeB.getParList();
				if (FOOLlib.isSubtype(arrowTypeA.getRet(), arrowTypeB.getRet())) { // co-varianza sul tipo di ritorno
					if (parListA.size() == parListB.size()) { // stesso numero di parametri
						boolean controVariancyFlag = true;
						for (int i = 0; i < parListA.size(); i++) {
							if (!(FOOLlib.isSubtype(parListB.get(i), parListA.get(i)))) { // contro-varianza sul tipo di ogni parametro
								controVariancyFlag = false;
							}
						}
						if (controVariancyFlag) {
							return true;
						}
					}
				}

			/* Controllo sui tipi ne' riferimento ne' funzionali... quindi bool o int */
		...
		private static boolean classIsSubtype(RefTypeNode a, RefTypeNode b) {
			if(a.getID().equals(b.getID())) {
				return true;
			}
			String check = superType.get(a.getID());
			while(check != null && !check.equals(b.getID()) ) {
				check = superType.get(check);
			}
		...
		public static Node lowestCommonAncestor(Node a, Node b) {
			if (a instanceof ArrowTypeNode && b instanceof ArrowTypeNode) {
				...
				ArrayList<Node> parListA = arrowTypeA.getParList();
				ArrayList<Node> parListB = arrowTypeB.getParList();
				ArrayList<Node> parListLowest = new ArrayList<Node>();
				if (parListA.size() == parListB.size()) {
					Node returnLowest = FOOLlib.lowestCommonAncestor(arrowTypeA.getRet(), arrowTypeB.getRet());
					if (returnLowest != null) {
						for (int i = 0; i < parListA.size(); i++) {
							if (FOOLlib.isSubtype(parListA.get(i), parListB.get(i))) {
								parListLowest.add(parListA.get(i));
							} else if (FOOLlib.isSubtype(parListB.get(i), parListA.get(i))) {
								parListLowest.add(parListB.get(i));
							} else {
								return null;
							}
						}
						return new ArrowTypeNode(parListLowest, returnLowest);
					}
				}
			} else if ((a instanceof EmptyTypeNode || a instanceof RefTypeNode) && (b instanceof EmptyTypeNode || b instanceof RefTypeNode)) {
				if (a instanceof EmptyTypeNode) {
					return b;
				} else if (b instanceof EmptyTypeNode) {
					return a;
				} else {  // sia a che b sono RefTypeNode
					String sa = ((RefTypeNode)a).getID();
//					System.out.println("sb ="+ ((RefTypeNode)b).getId());
					RefTypeNode ancestor;
					while (sa != null) {
//						System.out.println("sa ="+ sa);
						ancestor = new RefTypeNode(sa);
						if (FOOLlib.isSubtype(b, ancestor)) {
							return ancestor;
						}
						sa = superType.get(sa);
		...
		public static void putCode(String c) {
			funCode += "\n" + c; // aggiunge una linea vuota di separazione prima del codice di metodi e funzioni
		}
		...
		public static void addSuperclass(String subclass, String superclass) {
			superType.put(subclass, superclass);
		}
		
		public static void addDispatchTable(ArrayList<String> classMethodList) {
			dispatchTables.add(classMethodList);
		}
